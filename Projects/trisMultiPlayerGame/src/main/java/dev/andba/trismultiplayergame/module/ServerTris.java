...
public class ServerTris implements Runnable {
    ...
    private void handleRead(SelectionKey key) {
        ...
        try {
            ...
            else if (operation.getAction().equals("Occupied")){
                handleOccupied(operation.getUser(),operation.getCompetitor());
                return;
            }else if (operation.getAction().equals("Decline")){
                handleDecline(operation.getUser(),operation.getCompetitor());
                return;
            }else if (operation.getAction().equals("Accept")){
                handleAccept(operation.getUser(),operation.getCompetitor());
                return;
            }else if (operation.getAction().equals("Moves")){
                handleMoves(operation.getUser(),operation.getCompetitor(), operation.getMoves());
                return;
            }
            else if (operation.getAction().equals("Win")){
            handleWin(operation.getUser(),operation.getCompetitor(), operation.getMoves());
            return;
            }
    ...
    private String handleGetOnlinePlayer(String user) {
        ...
        serverResponse.setOperation("GetOnlinePlayer");
        List<String> playerList = new ArrayList<>();
        for(String temp : userChannels.keySet()) {
            if (!temp.equals(user))
                playerList.add(temp);
        }
        serverResponse.setData(playerList);
        try {
    ...
    private void handleRequestGame(User user, String competitor) {
        sendServerMessage(user, competitor, "RequestGame", user.getUsername());
    }

    private void handleOccupied(User challenged, String challenger) {
        sendServerMessage(challenged, challenger, "RequestResponse",
                "L'utente: " + challenged.getUsername() + " è già impegnato.");
    }

    private void handleAccept(User challenged, String challenger) {
        sendServerMessage(challenged, challenger, "RequestResponse", "Accepted");
    }

    private void handleDecline(User challenged, String challenger) {
        sendServerMessage(challenged, challenger, "RequestResponse", "Refused");
    }

    private void handleMoves(User user, String competitor, int[] moves) {
        sendServerMessage(user, competitor, "Moves", Arrays.toString(moves));
    }

    private void handleWin(User user, String competitor, int[] moves) {
        sendServerMessage(user, competitor, "Lose", "");
    }

    private void sendServerMessage(User receiver, String targetUsername, String operation, String messageContent) {
        SocketChannel channel = userChannels.get(targetUsername);
        ServerResponse<String> serverResponse = new ServerResponse<>();
        serverResponse.setOperation(operation);
        serverResponse.setData(messageContent);
        try {
            String message = objectMapper.writeValueAsString(serverResponse);
            writeBuffer.clear();
            ...
            userChannels.remove(receiver.getUsername());
        }
    }
